...
> **回写更正（2026-09-13）**——上表三处标注已过时，按实况更正如下：
>
> | 原标注 | 更正为 | 依据 |
> |---|---|---|
> | `多假设交叉验证` → `待 P1-1 并行化` | **标注错误**：P1-1 并行化**已完成**（见本文件 L233 `✅`）。该行真正的问题是多假设**只覆盖 2 个维度**，属覆盖度不足，与并行化无关 | `run_executor_all` + `parallel_executor_workers` |
> | 三行 `待 Analyst / Reporter 真实模型` | **不再是"无模型可用"**：已有可用真实端点（Matrix / `deepseek-v4-flash-w8a8`）并产出**首个有效真实基线**（7/7 计分、0 降级）。**但这三行针对的 `sleep.csv` 案例本身仍未重跑**，故结论仍为**未验证** | `docs/progress/eval-real-analyst-dataset.md` |
> | 上方"回归测试状态"表的 `349 项 / 376 收集` | **历史快照**，非当前基线。当前为 **856 passed / 0 failed / 15 skipped** | `docs/progress/metrics.md` |
>
> 即：**"缺一个能用的模型"这一前置条件**已经解除；剩下的是**这个案例没重跑**，
> 以及"洞察深度"在真实模型下究竟如何，需用新端点重跑 `sleep.csv` 才能回答。
