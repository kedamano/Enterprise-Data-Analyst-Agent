...
| 现状 | 差距（原始判断） | 状态 | 复核证据 |
|---|---|---|---|
| SQL 只读 + 白名单 + 沙箱 + 审计 JSONL + Prompt 注入分层 | 缺 **RBAC / 细粒度权限**；缺 **两步授权 / 高危确认 HITL**；审计为文件非不可篡改库；无间接注入对抗集；**无输出 PII 脱敏** | **⚠️ 部分** | RBAC ✅（AUTH/01 用户级鉴权 + 角色→权限门禁，`test_rbac.py` 11 + `test_auth_permissions.py` 18）；细粒度权限 ✅（表/列/行三级）；**输出 PII 脱敏 ✅**（E4/02 三级 + 值形态兜底 + **失败即关闭**，并写审计）；SQL 只读守卫已加固（`test_sql_readonly_guard.py` 33 项）；**两步授权 HITL ✅**（D45：**默认拒绝 + 显式白名单** + fail-closed + 凭证不可推导；`export_raw`/`deliver_python` 需 `POST /analyze/confirm`；允许与拒绝**都审计**；默认关=零影响）；**审计落库 ✅**（D46：四条流统一经 `security/audit_store.py`，`jsonl`/`sqlite`/`postgres` 三选一；`import_jsonl()` **内容指纹保证幂等**；`GET /debug/audit` SQL 查询）。**残留**：间接注入对抗测试集 |
...
2. **安全合规最硬两件**：两步授权 HITL、审计落库（非 JSONL）。
3. **镜像瘦身实测 + 镜像仓库 + schema 版本迁移**（CI 与 `<2GB` 断言已就位，卡在镜像源）。

**P1 · 撑故事 / 加分**
4. **prompt 预算强制 + 单会话 token 熔断**（唯一"说了但完全没做"的工程项）。
5. 长任务异步化；日志采样 / SLI-SLO；多租户隔离**测试矩阵**。
6. 检索侧：query 改写、多跳拆分、低置信 fail 兜底。
7. 业务标注集 / 人工评审 / badcase 回流；幻觉率监控。
