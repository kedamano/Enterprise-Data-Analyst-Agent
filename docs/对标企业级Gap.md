...
至 D57 复核：**22 条中 6 条完全消除、13 条只剩残项、3 条仍成立**（§一
「tool_choice」「子Agent」、§六「异步任务」）。**真正还差的三块半**是：**真实业务数据验证
（真业务库 + 真业务问句）、可运维部署体系（镜像体格 + 真实业务数据）、检索深度与数据治理
（跨副本一致性、embedding 版本迁移、向量与源文档权限对齐、版面理解 / OCR / 父子 chunk），
